...
export default function AdminOrders() {
    ...
    const dataUrlToJpg = (dataUrl: string): Promise<string> => {
        return new Promise((resolve) => {
            ...
            img.onload = () => {
                ...
                // Fill white background (JPG does not support transparency)
    ...
    const handleDownloadImage = async (order: Order) => {
        // 1. Download print-ready PNG
        if (order.printImage) {
            downloadFile(order.printImage, `order-${order.id}-PRINT.png`);
        }

        // 2. Download mockup/preview as JPG (with a small delay to avoid browser blocking)
        if (order.previewImage) {
            await new Promise(r => setTimeout(r, 400));
            const jpgDataUrl = await dataUrlToJpg(order.previewImage);
            downloadFile(jpgDataUrl, `order-${order.id}-MOCKUP.jpg`);
        }
    };
