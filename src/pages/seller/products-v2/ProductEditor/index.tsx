                        ...
                        .map((stepNum) => {
                          ...
                          return (
                            <div key={stepNum} className="border border-gray-200 rounded-xl overflow-hidden bg-white shadow-sm">
                              ...
                              {!isCollapsed && (
                                <div className="p-4 flex flex-col gap-3 animate-in fade-in slide-in-from-top-2 duration-200">
                                  {(() => {
                                    const groupsByCategory: Record<string, any[]> = {};
                                    groups.forEach(g => {
                                      const cat = g.ui_config?.category || '未分類';
                                      if (!groupsByCategory[cat]) groupsByCategory[cat] = [];
                                      groupsByCategory[cat].push(g);
                                    });

                                    // Sort categories: put '未分類' at the end, others by categorySortOrder or alphabetically
                                    const sortedCategories = Object.keys(groupsByCategory).sort((a, b) => {
                                      if (a === '未分類') return 1;
                                      if (b === '未分類') return -1;

                                      const orderA = groupsByCategory[a][0]?.ui_config?.categorySortOrder ?? 999;
                                      const orderB = groupsByCategory[b][0]?.ui_config?.categorySortOrder ?? 999;
                                    ...
                                    return sortedCategories.map(cat => {
                                      ...
                                      return (
                                        <div key={cat} className="border border-gray-200 rounded-lg overflow-hidden bg-white shadow-sm">
                                          <button
                                            onClick={() => {
                                              setActiveCategoryByStep(prev => ({
                                                ...prev,
                                                [stepNum]: prev[stepNum] === cat ? null : cat
                                              }));
                                            }}
                                          ...
                                          {isCatExpanded && (
                                            <div className="p-3 grid grid-cols-1 md:grid-cols-2 gap-3 bg-white animate-in fade-in slide-in-from-top-1 duration-200">
                                              {catGroups
                                                .sort((a, b) => (a.ui_config?.sortOrder || 0) - (b.ui_config?.sortOrder || 0))
                                                .map((group) => {
                                                  const linkedGroups = draft?.specs?.linked_option_groups || [];
                                                  const isChecked = linkedGroups.includes(group.id);

                                                  const groupItems = optionItems.filter((i: any) => (i.parentId || i.parent_id) === group.id);
                                                        ...
                                                        <div className="p-3 bg-white border-t border-blue-100">
                                                          ...
                                                          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2">
                                                            {groupItems.map((item: any) => {
                                                              ...
                                                              return (
                                                                <label key={item.id} className={`flex items-center gap-2 p-1.5 border rounded cursor-pointer text-xs transition-colors ${isItemSelected ? 'bg-blue-50/50 border-blue-300 shadow-sm' : 'bg-gray-50 border-gray-200 opacity-60 hover:opacity-100 hover:border-gray-300'}`}>
                                                                  <input
                                                                    type="checkbox"
