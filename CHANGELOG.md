...
## [10.1] - 2026-03-25
### Features & Fixes
- **一鍵下載與搜尋優化 (Order Management)**：後台訂單管理介面搜尋輸入現已支援忽略前後空格，並加入快速清除 (X) 功能。列表區塊新增了「一鍵下載所有圖片」的綠色按鈕，能依序批次下載 列印用大圖 (Print)、預覽圖 (Preview) 以及客戶 AI 規格截圖 (Spec)，大幅加快客服接單流程。
- **模板建立器背景縮放修復 (Template Builder Fixes)**：解決了在「建立設計款模板 (Admin)」模式下，由於缺乏手機基準外殼圖層導致純色背景無法套用，以及圖片背景縮小至左上角的問題。程式碼現已全面採用邏輯真實尺寸 (`REAL_WIDTH` / `REAL_HEIGHT`) 進行滿版 100% 背景套用與計算。
...
## [9.6] - 2026-03-22
### Features & Fixes
- **行動版全螢幕面板**：優化手機版前台介面，將「貼圖 / 背景 / 相框 / 設計款」的底部滑動面板全部改為滿版全螢幕設計（會蓋過整頁的背景），點擊背景或點選任何素材後即自動關閉視窗，使操作體驗更順暢不會擋住畫面。
- **加購明細價格修正**：修復了左側加購明細與卡片上的加價顯示邏輯。現在會正確將 Option Group 的基礎加價納入總額；前台的子選項卡片如果是免費款式，則隱藏其專屬加價金額，避免將基礎價合併顯示造成使用者混淆。
- **後台子項加價金額移除**：針對後台規格管理的「顏色 / 子項」，移除了「加價金額」功能與輸入欄位，使其專注於純顏色/款式選擇，真正的加價統一交由「大類基礎價」與「附加選項」管理。

## [9.5] - 2026-03-22
### Features
- **後台素材庫操作優化**：
  - **行內編輯**：現在滑鼠移至圖片下方的名稱文字，即可直接編輯修改，按下 `Enter` 或游標移開 (onBlur) 自動存檔，無須再個別點開編輯視窗。
  - **批次分類切換**：新增多選核取框與批次操作工具列。可以一次勾選多張圖片，並將它們一鍵分配至指定分類中。

## [9.4] - 2026-03-22
### Bug Fixes
- **首頁白屏緊急修復**：強化 `getOptimizedUrl` 針對不明型別變數（例如非字串之內容）的防呆處理 (`try-catch` / typeof 檢查)，避免 React 組件因無法解析 URL 網址而在渲染期間崩潰導致畫面白屏。並執行完整重新部署強迫清理快取 (Cache Busting)。

## [9.3] - 2026-03-22
### Features
- **AI 辨識防呆專屬視窗**：當客戶上傳的是純照片而非規格截圖導致辨識失敗時，系統現會攔截錯誤並統一彈出一個「截圖無效」的專屬警告彈窗。
- **試衣間引導按鈕**：在這個防呆彈窗中，加入了十分醒目的紅色按鈕「🔴 Devilcase 配色試衣間（點我開啟）」，引導客戶直接新分頁前往官網完成搭配再重新截圖，減少跳離率與客服負擔。

## [9.2] - 2026-03-22
### Features
- **AI 規格辨識防呆機制**：優化後端 AI 辨識指令 (`recognize-product.js`)，阻擋無規格文字的照片上傳（如純商品圖、動物照片），並透過 `aiRecognition.ts` 在前台進行二重檢查與警告，協助顧客確實上傳規格文字截圖。
- **網站圖片載入大幅加速 (Image Proxy)**：新增 `api/image-proxy` Vercel 節點與 `imageOptimizer.ts`。自動為「首頁全部商品」與「設計模板」產生動態壓縮與 WebP 轉換。解決過去因直接載入原始肥大檔案導致首頁卡頓打開偏慢的問題。
- **手機版預覽圖滑動支援**：優化 `ProductPreviewPanel.tsx`，加入觸控左右滑動 (Swipe) 以及直接點選圖片即可切換下一張圖的功能，提升移動端用戶尋找與預覽商品的流暢度。
- **設計保存彈窗 UI 精簡化**：優化首頁「設計已保存」彈出視窗，縮小「複製設計ID」按鈕並整合，引導使用者更直覺點選下一步選擇規格。

## [9.1] - 2026-03-22
### Features
- **素材庫批量刪除功能**：在「素材庫管理」中新增了多選框與「刪除所選」按鈕。修復了因 URL 過長導致的 Delete 失敗問題，改採並行小批刪除且直接更新畫面狀態，並移除舊有會被 HMR 中斷的 `window.confirm`，改用內嵌的確認橫幅。
- **素材庫批量上傳功能**：上傳區域現在支援一次選取多張圖片（PNG, JPG, WebP）。系統會在背景顯示上傳進度，並依序彈出「編輯素材資訊」視窗，透過按鈕上的自動計數提示（如「儲存並開啟下一張 (3)」）讓賣家能連續快速修改所有新素材的名稱與標籤。
- **類別管理 Inline 編輯**：將原本只能在側邊面板編輯的「類別列」，大幅升級為在 Tab 列上直接互動的功能。支援拖曳左側把手重新排序、雙擊類別名稱直接就地改名、加入右上角直接新增按鈕，以及 Hover 時顯示刪除按鈕（「未分類」受保護無法刪除），體驗極致直覺。

## [9.0] - 2026-03-20
### Features
- **AI 創意點數重置機制 (AI Rolling Hours)**：將 AI 點數重置計算由統一的每日截止時間，升級為更靈活的「滾動週期間隔 (Rolling Hours)」。現在系統將從使用者「首次消耗點數」的時間開始起算 N 小時（可由後台自訂，如 24 小時）。
- **全域 AI 設定介面優化**：後台 `AiStylePresets` 畫面重新設計，將「儲存設定」與「新增風格」按鈕完美垂直對齊，且以全新的白色卡片式元件整合「AI 點數重置週期」選單，提升了系統整體美感與一致性。

### Bug Fixes
- **防禦 CORS 遺失影像 (SecurityError)**：修復設計管理中「開啟編輯」時原本會遇到 `Fabric.js + base64 data URL + crossOrigin='anonymous'` 產生的 CORS 安全問題。直接對 Base64 影像停止綁定 crossOrigin 參數，讓被保存於設計檔內的圖層能正常還原不遺漏。
- **訂單設計建立時間修正**：修復了 `Orders.tsx` 介面顯示設計時間會錯誤抓取本地時區的 Bug，現已統一為 `{ timeZone: 'Asia/Taipei' }` 台灣時間顯示。
- **AI 拼貼提示詞比重調整**：修復之前 AI 生成過度偏向主題文字的狀況。更新 prompt 生成邏輯，強勢限定「30% 的視覺比重嚴格保留給所選的版面風格」，達成主題與風格的 70/30 完美平衡。

## [8.9] - 2026-03-19
### Features
- **前台介面 (Frontend UI)**：全新 Apple 風格的素材標籤雲與搜尋列，並改為懸浮 (Popover) 設計，僅在搜尋焦點時出現，釋放更多素材顯示空間。
- **素材庫管理 (Admin Assets)**：新增「自動帶入分類」功能，上傳貼圖、背景、相框時，會自動繼承上方過濾選取的分類標籤。

### Bug Fixes
- **AI 素材儲存**：徹底修復使用「去背/卡通化」等 AI 功能後，圖片在後台無法顯示的 Bug（改由畫布二次擷取並安全備份至 Supabase），防止暫存網址過期。
- **前台背景圖片顯示**：增加 `onError` 備援機制，解決新上傳的背景在生成縮圖失敗時出現破圖的問題（自動退回使用原圖）。
- **相框搜尋**：補上 `tags` 資料關聯，修復相框無法使用標籤搜尋的錯誤。
- **編輯列解鎖按鈕**：修復「解鎖/鎖定」按鈕在選取一般圖片或貼圖（帶有遮罩時）會錯誤顯示的問題，現已嚴格限制該功能僅於點擊「相框」時出現。

## [8.8] - 2026-03-19
### Features
- **SaaS 工具列自訂功能 (SaaS Toolbar Customization)**：
  - **後台管理頁面**：新增「工具列設定」頁面，支援全局控管前台工具列項目的顯示/隱藏、自訂名稱以及拖拽排序。
  - **同步與套用邏輯**：實作「一鍵套用至所有商品」機制，確保全站工具列風格統一，大幅降低維運成本。
  - **上傳文案與彈窗排版優化**：
    - 支援針對「我的圖庫」圖片上傳彈窗設定自訂的授權聲明文字與條款連結。
    - 優化彈窗內文排版，將「服務條款」連結獨立顯示於第二列，提升法律聲明的可讀性與美感。
- **前台動態渲染核心**：重構 `Home.tsx` 工具列渲染邏輯，現在會依據 `specs.toolbar_config` 動態生成對應的工具按鈕與權限判斷，支援即時配置更新。

## [8.7] - 2026-03-19
### Features
- **AI 創意生成優化**：
  - **自動翻譯+擴展主題**：整合 OpenAI GPT-4o-mini，現在支援輸入中文主題（如「京都」、「富士山」），系統會自動將其翻譯並擴展為豐富的英文視覺描述，大幅提升 AI 對特定主題的理解與生成效果。
  - **高品質背景生成**：將背景生成模型由 `flux-schnell` 升級為 `flux-1.1-pro`，解析度更高、細節更精緻，達到高品質印刷標準。
- **相框照片同步縮放**：實作相框與內部照片的聯動機制。現在移動、旋轉、或等比例縮放相框時，嵌入其中的照片與裁切區域會同步調整，保持完美的視覺比例，不再發生照片脫離框線的狀況。

### Bug Fixes
- **AI 點數扣除邏輯修正**：
  - 修復 `onCheckAndIncrementUsage` 點數扣除與顯示不同步的問題。
  - 修正 fallback 邏輯中的點數上限（由 10 點提升至 20 點），與後端最新設定完全對齊。
  - 優化點數不足的邊際判斷 (Off-by-one bug fix)，確保剩餘點數剛好等於消耗點數時仍能正常使用。

---

## [8.6] - 2026-03-19
### Features
- **空相框自動彈出圖庫**：在前台點擊設計模板中尚未填入照片的空相框時，系統現在會自動觸發「我的圖庫」彈窗，簡化使用者操作流程。

### Bug Fixes
- **強化相框屬性恢復與向後相容性**：
  - 修復 `restoreFromJSON` 在套用邊界裁切時可能意外覆蓋相框自訂屬性的問題。
  - 新增針對舊版格式模板 (`data.kind === 'frame'`) 的自動識別與修復機制，確保所有版本的設計模板相框功能均能正常運作。
  - 修正 Fabric.js 事件監聽器中的 closure stale 問題，改用 Ref 模式確保彈窗功能穩定。

---

## [8.5] - 2026-03-18
### Features
- **管理員導覽選單資料庫同步 (SaaS 準備)**：將管理後台的選單排序與自訂名稱由 `localStorage` 遷移至 Supabase `admin_preferences` 資料表。實作「本地快取 + 資料庫同步」雙層策略，確保同一帳號在不同瀏覽器、不同裝置下的排版完全一致，並為未來多租戶 (Multi-tenant) 訂閱架構奠定基礎。
- **後台模板建立器畫布優化**：
  - 加大設計區域比例，移除多餘的間距，讓畫布在工作區內達到同比例最大化，提升創作視覺精確度。
  - 新增畫布「透明格子 (Checkerboard)」背景，同時維持外圍容器為純白色，更清晰辨識透明圖層邊界。
- **後台工具列位置調整**：將原本漂浮在畫布上方的「圖層/復原/重做/清空」工具列移入右側側邊欄頂部，優化 PC 版佈局。

### Bug Fixes
- **修復相框屬性序列化遺失**：修正 Fabric.js 在匯出/匯入設計時導致 `isFrameLayer` 與相框專屬 ID (FrameId) 遺失的問題。改用強健的 metadata 注入系統，確保模板中的相框在轉存後能持續保有「雙擊加照片」的功能。

---
## [8.4] - 2026-03-17
### Features
- **產品選項子項 (款式/顏色) 獨立勾選功能**：在「管理後台 > 產品模板 > 編輯產品 > 屬性與關聯規格」中，現可依據單一產品需求獨立設定「可用的顏色或款式 (Option Items)」。系統新增 `linked_option_items` 資料結構支援此邏輯，不僅讓前台 `SaveDesignModal` 中的選單精準過濾掉未開放的選項，亦做到舊有資料完美向後相容（未設定均預設為全開）。

---

## [8.3] - 2026-03-17
### Features
- **系統檔案清理功能**：在後台「系統設定」(`SystemSettings.tsx`) 新增支援掃描與清理孤立圖像檔案(Orphaned files)的功能介面。
- **產品分類狀態保留**：在「產品管理 V2」(`ProductListV2`) 中，將選取的分類狀態改以 URL 參數 (`?cat=`) 保存，確保從商品編輯頁面返回時，列表不會重置為「全部」。
